...
class SewNeedleEnv(GuidedVisionEnv):
    ...
    def __init__(self, **kwargs):
        # 数据收集专用成功标准：比训练/普通评估环境更严格。
        # 这些 kwargs 会先取出，避免传给 GuidedVisionEnv 后触发 unexpected keyword。
        self.strict_success_x_threshold = float(kwargs.pop("strict_success_x_threshold", 0.010))   # X 方向对中误差必须小于 1.2 厘米
        self.strict_success_y_threshold = float(kwargs.pop("strict_success_y_threshold", 0.035))   # Y 方向对中误差必须小于 3.5 厘米（考虑到缝合针较长，允许一定的 Y 方向偏移）
        self.strict_success_z_margin = float(kwargs.pop("strict_success_z_margin", 0.00))         # Z 方向抬举高度必须超过孔口至少 0.5 厘米，确保真正过孔并抬起，而不是卡在洞口上。
        self.strict_success_hold_steps = int(kwargs.pop("strict_success_hold_steps", 2))           # 成功条件需要连续保持的步数，过滤一帧误触发。
        self.strict_success_require_right_release = bool(
            kwargs.pop("strict_success_require_right_release", True)
        )
        ...
        self._strict_success_hold_count = 0
        self.strict_success_metrics = {}
    ...
    def _calculate_distances(self):
        ...
        z_clearance = needle_center[2] - target_z_target
    ...
    def reset(self, seed=None, options=None) -> tuple:
        ...
        self._strict_success_hold_count = 0              # 严格成功条件连续保持计数
        self.strict_success_metrics = {}
        ...
        if not self.left_has_grasped:
            ...
            if not touch_left_gripper:
                ...
                if not self.needle_completely_through:
                    ...
                    # 数据收集专用：只有更干净的末端状态才判成功。
                    # 1. 针已经完全过孔；
                    # 2. 针中心在墙中心 X 方向更精确对齐；
                    # 3. Y 方向不能偏得太大；
                    # 4. 抬举高度需要超过普通阈值一点余量；
                    # 5. 默认要求右手已经松开，避免保存双手/碰撞拖拽的轨迹；
                    # 6. 条件需要连续保持数步，过滤一帧误触发。
                    right_release_ok = (not touch_right_gripper) or (
                        not self.strict_success_require_right_release
                    )
                    strict_success_pose = (
                        curr_dists['x_error'] < self.strict_success_x_threshold
                        and curr_dists['y_error'] < self.strict_success_y_threshold
                        and curr_dists['z_clearance'] >= self.strict_success_z_margin
                        and right_release_ok
                    )
                    if strict_success_pose:
                        self._strict_success_hold_count += 1
                    else:
                        self._strict_success_hold_count = 0
                    ...
                    self.strict_success_metrics = {
                        ...
                        "z_clearance": float(curr_dists["z_clearance"]),
                        "right_release_ok": bool(right_release_ok),
                        "hold_count": int(self._strict_success_hold_count),
                        "hold_required": int(self.strict_success_hold_steps),
                    }

                    if self._strict_success_hold_count < self.strict_success_hold_steps:
                        # 引导向上抬举并对中
                        progress = self._prev_dists['composite_error_dist'] - curr_dists['composite_error_dist']
                        reward += diff_scale * progress
                        if strict_success_pose:
                            reward += 5.0 * self._strict_success_hold_count
                    else:
                        reward += 500.0
